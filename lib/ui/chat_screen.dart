...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  void scrollDown() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ref.read(completionProvider).controller.jumpTo(
          ref.read(completionProvider).controller.position.maxScrollExtent);
    });
  }
                  ...
                  itemBuilder: (context, i) {
                    return Column(
                      children: [
                        ...
                        ChatParagraphTile(
                          text: data[i].isQuestion
                              ? data[i].question
                              : data[i].answer,
                          leadingIcon: data[i].isQuestion
                              ? const ProfileIcon()
                              : const ChatGpt(),
                        ),
                      ],

                  ...
                  : InkWell(
                      ...
                      onTap: () async {
                        ref
                            ...
                            .pushQuestionAnser(true, controller!.text);
                        ref.read(completionProvider).setLoading(true);
                        if (controller!.text.isNotEmpty) {
                          try {
                            OpenAICompletionsResponse? res =
                                await getAnswer(controller!.text);
                            ref.read(completionProvider).pushQuestionAnser(
                                false, res?.choices?[0].text ?? "");
                          } catch (_) {}
                        }
        ...
        onFieldSubmitted: (_) async {
          ...
          if (controller!.text.isNotEmpty) {
            try {
              ...
              ref
                  .read(completionProvider)
                  .pushQuestionAnser(false, res?.choices?[0].text ?? "");
            } catch (_) {}
          }
          ref.read(completionProvider).setLoading(false);
        },
